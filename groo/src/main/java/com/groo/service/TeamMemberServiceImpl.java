...
public class TeamMemberServiceImpl implements TeamMemberService {
	...
	public void updateTeamPage(TeamMemberDTO teamMember) {
		...
		try {
			dao.updateTeamPage(teamMember, session);
			session.commit();
		} catch (InternalDataAccessException ide) {
			ide.printStackTrace();
			throw new InternalServiceException("DB 접근 오류로 인한 서비스 예외", ide);
	...
	public void insertTeamPage(TeamMemberDTO teamMember) {
		...
		try {
			dao.insertTeamPage(teamMember, session);
			session.commit();
		} catch (InternalDataAccessException ide) {
			ide.printStackTrace();
			throw new InternalServiceException("DB 접근 오류로 인한 서비스 예외", ide);
	 ...
	 */
	@Override
	public void deleteTeamPage(TeamMemberDTO teamMember) {
		SqlSession session = SessionFactory.getSqlSession();

		try {
			dao.deleteTeamPage(teamMember, session);
			session.commit();
		} catch (InternalDataAccessException ide) {
			ide.printStackTrace();
			throw new InternalServiceException("DB 접근 오류로 인한 서비스 예외", ide);
