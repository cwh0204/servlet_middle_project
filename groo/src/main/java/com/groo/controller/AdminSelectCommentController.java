...
/**
 * 관리자 페이지에서 댓글 검색을 처리하는 Controller 구현 클래스 Service 계층을 호출하고 처리 결과를 json 형태로
 * 클라이언트에 응답
 */
public class AdminSelectCommentController implements Controller {
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			comentList = service.adminSelectComment(coment);
			Gson gson = new Gson();
			String json = gson.toJson(comentList);
