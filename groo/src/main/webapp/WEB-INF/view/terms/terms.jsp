...
이 약관은 groo (이하 '회사' 라고 합니다)가 제공하는 제반 서비스의 이용과 관련하여 회사와 이용자와의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
...
2.  '이용자'란 이 약관에 따라 회사가 제공하는 서비스를 받는 '회원' , '비회원'을 말합니다.
3.  '회원'은 회사에 개인정보를 제공하여 회원등록을 한 사람으로, 회사로부터 지속적으로 정보를 제공받고 '회사'가 제공하는 서비스를 계속적으로 이용할 수 있는 자를 말합니다.
4.  '비회원'은 회원가입 없이 회사가 제공하는 서비스를 이용하는 자를 말합니다.
5.  '아이디(ID)'라 함은 회원의 식별과 서비스이용을 위하여 회원이 정하고 회사가 승인 하는 문자 또는 문자와 숫자의 조합을 의미합니다.
...
<strong>제 4 조(약관의 효력과 변경)</strong>
1.  이 약관은 groo(이)가 제공하는 모든 인터넷서비스에 게시하여 공시합니다. '회사'는 '전자상거래 등에서의 소비자보호에 관한 법률(이하 '전자상거래법'이라 함)', '약관 의 규제에 관한 법률(이하 '약관규제법'이라 함)', '전자문서 및 전자거래 기본법(이하 '전자문서법'이라 함), '전자금융거래법 , '정보통신망 이용촉진 및 정보보호 등에 관 한 법률(이하 '정보통신망법'이라 함), '소비자기본법' 등 관계 법령(이하 '관계법령' 이라 함)에 위배되지 않는 범위 내에서 이 약관을 변경할 수 있으며, 회사는 약관이 변경되는 경우에 변경된 약관의 내용과 시행일을 정하여, 그 시행일로부터 최소 7일 (이용자에게 불리하거나 중대한 사항의 변경은 30일) 이전부터 시행일 후 상당한 기간 동안 공지하고, 기존 이용자에게는 변경된 약관, 적용일자 및 변경사유(변경될 내용 중 중요사항에 대한 설명을 포함)를 별도의 전자적 수단(전자우편, 문자메시지, 서비스 내 전자쪽지발송, 알림 메시지를 띄우는 등의 방법)으로 개별 통지합니다. 변경된 약관은 공지하거나 통지한 시행일로부터 효력이 발생합니다.
...
<strong>제 5 조(이용자에 대한 통지)</strong>
...
<strong>제 6 조(이용계약의 체결)</strong>
이용계약은 다음과 같은 경우에 체결됩니다.
1.  이용자가 약관에 동의하고 회원가입을 완료한 후, 회사가 회원가입을 승인한 경우
2.  회사가 회원가입 없이 이용할 수 있도록 정한 일부 서비스를 비회원이 이용하는 경우

	
<strong>제 7 조(회원가입에 대한 승낙)</strong>
1.  회사는 이용계약에 대한 요청이 있을 때 서비스 이용을 승낙함을 원칙으로 합니다.
2.  제1항에 따른 신청에 있어 회사는 서비스 제공에 필요한 경우 전문기관을 통한 본인인증을 요청할 수 있습니다.
...
5.  이용계약의 성립 시기는 회원가입 절차가 완료되고 회사가 가입 완료를 표시한 시점으로 합니다.
6.  회사는 서비스 이용 활성화를 위해 회원의 활동 수준이나 참여도에 따라 뱃지 부여 등 다양한 혜택을 제공할 수 있습니다.

<strong>제 8 조(회원정보의 변경)</strong>
...
<strong>제 9 조(회원정보의 관리 및 보호)</strong>
...
2.  회사는 이용계약의 체결, 계약사항의 변경 및 해지 등 이용자와의 계약관련 절차가 내용 등에 있어 이용자에게 편의를 제공하도록 노력합니다.

<strong>제 11 조(개인정보보호)</strong>
...
3.  회사가 제공하는 서비스의 내용은 다음과 같습니다.
  가. 웹페이지를 이용하여 제공되는 서비스 
  나. 이용자의 편의를 위하여 제공하는 서비스

<strong>제 14 조(서비스의 제한 등)</strong>
...
2.  회사는 서비스의 이용을 제한하거나 정지하는 때에는 그 사유 및 제한기간, 예정 일시 등을 지체없이 이용자에게 알립니다.

<strong>제 15 조(서비스의 해제•해지 및 탈퇴 절차)</strong>
1.  이용자가 이용 계약을 해지하고자할 때는 언제든지 홈페이지 상의 이용자 탈퇴 신청을 통해 이용계약 해지를 요청할 수 있습니다. 단, 신규가입 후 일정 시간 동안 서비스 부정이용 방지 등의 사유로 즉시 탈퇴가 제한될 수 있습니다.
...
<strong>제 17 조(면책사항)</strong>
1.  회사는 천재지변 또는 이에 준하는 불가항력으로 인하여 서비스를 제공할 수 없는 경우에는 서비스 제공에 관한 책임을 지지 않습니다.
2.  회사는 이용자의 귀책사유로 인한 서비스 이용장애에 대하여 책임을 지지 않습니다.
3.  회사는 이용자가 웹페이지에 게재한 내용의 신뢰도, 정확성 등 내용에 대해서는 책임지지 않으며, 이용자 상호간 또는 이용자와 제3자 상호간 서비스를 매개로 발생한 분쟁에 개입하지 않습니다.

<strong>제18조(권리의 귀속)</strong>
1.  회사가 제공하는 서비스에 대한 저작권 등 지식재산권은 회사에 귀속됩니다.
2.  회사는 서비스와 관련하여 이용자에게 회사가 정한 조건 따라 회사가 제공하는 서비스를 이용할 수 있는 권한만을 부여합니다.
...
2.  제1항에 불구하고 회사는 서비스의 운영, 전시, 전송, 배포, 홍보 등의 목적으로 별도의 허락없이 무상으로 저작권법 및 공정한 거래관행에 합치되는 범위 내에서 다음 각 호와 같이 회원이 등록한 콘텐츠를 사용할 수 있습니다.
  가. 서비스의 운영, 홍보,서비스 개선 및 새로운 서비스 개발을 위한 범위 내의 사용

<strong>제21조(관할법원 및 준거법)</strong>
서비스와 관련하여 분쟁이 발생한 경우 관할법원은 회사 소재지 관할법원으로 정하며, 준거 법은 대한민국의 법령을 적용합니다.

<strong>부칙</strong>
제1조(시행일)
본 약관은 2025.11.12.부터 시행됩니다.
</pre>